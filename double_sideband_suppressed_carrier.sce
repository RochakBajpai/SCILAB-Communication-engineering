// DOUBLE SIDE BAND SUPPRESSED CARRIER MODULATION AND DEMODULATION...frequency spectrum of modulated signal
clc;
clear;
close;
...
t = 0: 0.01:3;
p = length(t);
m = 1*cos(2 *%pi *3*t);    // m(t) =Am cos(2 pi fm t)
subplot(2,1,1)
plot(t,m)
xlabel('time');
ylabel('Amplitude');
...
c = 2* cos(2 * %pi *20*t);     // c(t) =Ac cos(2 pi fc t)
figure(2)
subplot(2,1,1)
plot(t,c)
xlabel('time');
...
u = (1*cos(2 *%pi *3*t)).*(2* cos(2 * %pi *20*t));

//2*(1+ 0.5*cos(2 *%pi *3*t)).* cos (2*%pi*20*t);//element wise multiclation
figure(3)
subplot(2,1,1);
plot(t,u)
xlabel('time');
